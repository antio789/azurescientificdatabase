...
function getPretreatmentMainFields() {
    const stmt = db.prepare("select id, name from pretreatment_fields where pretreatment_fields.id not in (select child from pretreatment_field_tree);")
    return stmt.all();
}
...
function getReactorMainFields() {
    const stmt = db.prepare("select id, name from reactors where reactors.id not in (select child from reactor_tree);")
    return stmt.all();
}
...
function getArticlesInfo(fieldIds, categoryIds) {
    const articles = getArticles(fieldIds, categoryIds, filterType.PRETREATMENT);
    //layout of articles: [{...},{...}]
    let articlesInfo = [];
